myApp.service('AuthService', function ($http) {
    ...
    // this is for testing and works with /auth view
    self.userRegister = function (userObj) {
        console.log('in userRegister');
        return $http.post('/registration/user', userObj).then(function (response) {
    ...
    self.userRegistration = function (userObj) {
        console.log('in userRegister');
        return $http.put('/register/user', userObj).then(function (response) {
            console.log('user registration successful');
        }).catch(function (response) {
            console.log('Registration error: ', response);
        }); // end catch
